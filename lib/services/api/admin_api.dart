...
class AdminApi {
  ...
  // جلب الطلاب مع فلترة بالبحث (اسم أو ID)
...
Future<List<dynamic>> getHalqaStudents(int halqaId) async {
    try {
      final response = await _client.dio.get("halqas/$halqaId/students/");
      return response.data is List ? response.data as List<dynamic> : [];
    } on DioException catch (e) {
      throw _handleError(e, "فشل جلب قائمة طلاب الحلقة");
    }
  }
  ...
  Future<List<dynamic>> searchStudents(String query) async {
    // نستخدم الرابط persons مع باراميتر search المدمج في الجانغو [4]
    final response = await _client.get("persons/?role=student&search=$query");
    return response.data;
  }
Future<List<dynamic>> getQuranTests({
    required String endpoint, 
    Map<String, dynamic>? queryParameters,
  }) async {
    try {
      final response = await _client.dio.get(endpoint, queryParameters: queryParameters);
  ...
  /// ➕ إضافة اختبار قرآني جديد
  Future<void> createQuranTest(Map<String, dynamic> data) async {
    try {
      // يفضل استخدام الـ _client.dio مباشرة أو الـ wrapper المتوفر لديك لتوحيد المعايير
      await _client.dio.post("quran-tests/", data: data);
    } on DioException catch (e) {
      throw _handleError(e, "فشل إضافة الاختبار؛ تأكد من التناغم بين نوع الاختبار (SURAH/PART) والبيانات");
    }
  }

  /// 🔄 تعديل بيانات اختبار سابق (Partial Update)
  Future<void> updateQuranTest(int id, Map<String, dynamic> data) async {
    try {
      await _client.dio.patch("quran-tests/$id/", data: data);
    } on DioException catch (e) {
      throw _handleError(e, "فشل تعديل بيانات الاختبار");
    }
  }

  /// ❌ حذف اختبار نهائياً من السجلات
  Future<void> deleteQuranTest(int id) async {
    try {
      await _client.dio.delete("quran-tests/$id/");
    } on DioException catch (e) {
 ...
 Future<List<dynamic>> getNotifications() async {
  ...
  Future<void> updateNotification(int id, Map<String, dynamic> data) async {
    try {
      await _client.patch("notifications/$id/", data: data);
    } on DioException catch (e) {
      throw _handleCentralError(e, "فشل تعديل الإشعار");
    }
  }
