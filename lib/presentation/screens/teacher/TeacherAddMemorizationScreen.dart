...
class MemorizationSessionSheet extends StatefulWidget {
  ...
  const MemorizationSessionSheet({Key? key, required this.args})
    : super(key: key);

  @override
  State<MemorizationSessionSheet> createState() =>
      _MemorizationSessionSheetState();
}
...
class _MemorizationSessionSheetState extends State<MemorizationSessionSheet> {
  ...
  void _onPageTap(int pageNum) {
    setState(() {
      if (_selectedFromPage == null ||
          (_selectedFromPage != null && _selectedToPage != null)) {
  ...
  Widget build(BuildContext context) {
    ...
    if (teacherProvider.studentProgress != null &&
        !_isInitialPagesSet &&
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: teacherProvider.isProgressLoading
                    ...
                    const Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 4),
                          Text(
                            "اضغط لتحديد صفحة البداية ثم صفحة النهاية:",
                            ...
                            child: AnimatedContainer(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (isStart &&
                                      _selectedToPage != null &&
                                      _selectedFromPage != _selectedToPage)
                                    const Text(
                                      "البداية",
                                      style: TextStyle(
                                        fontSize: 9,
                        ...
                        children: [
                          DropdownButtonFormField<String>(
                            ...
                            decoration: const InputDecoration(
                              labelText: "تقدير التسميع اليومي",
                              border: OutlineInputBorder(),
                            ),
                            items: const [
                              DropdownMenuItem(
                                value: 'excellent',
                                child: Text("ممتاز (بدون أخطاء)"),
                              ),
                              DropdownMenuItem(
                                value: 'very_good',
                                child: Text("جيد جداً (1-2 أخطاء)"),
                              ),
                              DropdownMenuItem(
                                value: 'good',
                                child: Text("جيد (3-4 أخطاء)"),
                              ),
                              DropdownMenuItem(
                                value: 'acceptable',
                                child: Text("مقبول (متردد كثيراً)"),
                              ),
                            ],
                            onChanged: (value) =>
                          ...
                          ElevatedButton(
                            onPressed:
                                (teacherProvider.isProgressLoading ||
                                    _selectedFromPage == null)
                                ? null
                                : _submitData,
                            style: ElevatedButton.styleFrom(
                              padding: const EdgeInsets.symmetric(vertical: 14),
                            ),
                            child: teacherProvider.isDashboardLoading
                                ? const SizedBox(
                                    height: 20,
                                    width: 20,
  ...
  void _submitData() async {
    ...
    final Map<String, dynamic> payload = {
      ...
      "page_to":
          _selectedToPage ??
          _selectedFromPage, // إذا لم يختر صفحة نهاية يعتبرها صفحة واحدة
    ...
    bool success = await teacherProvider.addMemorization(payload, student.id);
    // bool success = true;/////just now while i cide teacher homr
    if (success && mounted) {
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("🚀 تم تحديث سجل الطالب وضخ النقاط في المحفظة بنجاح"),
        ),
      );
    }
  }
